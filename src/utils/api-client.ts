...
export class ApiClient {
  ...
  async get<T = any>(
    ...
    try {
      ...
      this.logger.success(
        `GET ${endpoint} completed in ${result.responseTime}ms`,
      );
      return result;
    } catch (error) {
  ...
  async post<T = any>(
    ...
    try {
      ...
      const result = await this.processResponse<T>(response, startTime);
      this.logger.success(
        `POST ${endpoint} completed in ${result.responseTime}ms`,
      );
      return result;
    } catch (error) {
  ...
  async put<T = any>(
    ...
    try {
      ...
      const result = await this.processResponse<T>(response, startTime);
      this.logger.success(
        `PUT ${endpoint} completed in ${result.responseTime}ms`,
      );
      return result;
    } catch (error) {
  ...
  async delete<T = any>(
    ...
    try {
      ...
      const result = await this.processResponse<T>(response, startTime);
      this.logger.success(
        `DELETE ${endpoint} completed in ${result.responseTime}ms`,
      );
      return result;
    } catch (error) {
  ...
  async patch<T = any>(
    ...
    try {
      ...
      const result = await this.processResponse<T>(response, startTime);
      this.logger.success(
        `PATCH ${endpoint} completed in ${result.responseTime}ms`,
      );
      return result;
    } catch (error) {
  ...
  validateStatus(
    ...
    if (isValid) {
      ...
      this.logger.error(
        `Status validation failed. Expected: ${expected}, Got: ${response.status}`,
      );
    }
      ...
      if (typeof schema === "object" && schema !== null) {
        for (const key in schema) {
          if (!(key in response.data)) {
            this.logger.error(
              `Schema validation failed: Missing property '${key}'`,
            );
            return false;
          }
        }
