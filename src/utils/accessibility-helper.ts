...
export class AccessibilityHelper {
  ...
  async runAxeScan(options?: {
    ...
    try {
      ...
      this.logger.info(
        `Axe scan completed: ${results.violations.length} violations found`,
      );
      return results;
    } catch (error) {
  ...
  async checkKeyboardNavigation(): Promise<{
    ...
    this.logger.info(
      `Keyboard navigation check completed: ${result.tabbableElements} tabbable elements, ${result.issues.length} issues`,
    );
    return result;
  }
  ...
  async checkColorContrast(threshold: number = 4.5): Promise<{
    ...
    this.logger.info(
      `Color contrast check completed: ${result.elements.length} elements checked`,
    );
    return result;
  }
  ...
  async checkImageAltText(): Promise<{
    ...
    this.logger.info(
      `Image alt text check completed: ${result.totalImages} images, ${result.issues.length} issues`,
    );
    return result;
  }
  ...
  async checkFormAccessibility(): Promise<{
      ...
      forms.forEach((form, formIndex) => {
        ...
        inputs.forEach((input, inputIndex) => {
          ...
          if (!label && !ariaLabel && !ariaLabelledby) {
            issues.push(
              `Form ${formIndex + 1}, Input ${inputIndex + 1}: No associated label`,
            );
          }
          ...
          if (input.hasAttribute("required")) {
            ...
            if (ariaRequired !== "true") {
              issues.push(
                `Form ${formIndex + 1}, Input ${inputIndex + 1}: Required field missing aria-required`,
              );
            }
          }
        });
    ...
    this.logger.info(
      `Form accessibility check completed: ${result.formsCount} forms, ${result.issues.length} issues`,
    );
    return result;
  }
  ...
  async checkHeadingStructure(): Promise<{
    ...
    const result = await this.page.evaluate(() => {
      ...
      headings.forEach((heading, index) => {
        ...
        if (level - previousLevel > 1) {
          issues.push(
            `Heading level jump from h${previousLevel} to h${level} at "${text}"`,
          );
        }
    ...
    this.logger.info(
      `Heading structure check completed: ${result.headings.length} headings, ${result.issues.length} issues`,
    );
    return result;
  }
  ...
  async checkARIALandmarks(): Promise<{
    ...
    const result = await this.page.evaluate(() => {
      ...
      if (mainElements.length === 0) {
        ...
        recommendations.push(
          "Multiple main landmarks found - should have only one",
        );
      }
    ...
    this.logger.info(
      `ARIA landmarks check completed: ${result.landmarks.length} landmarks found`,
    );
    return result;
  }
  ...
  async generateAccessibilityReport(): Promise<{
    ...
    this.logger.success(
      `Accessibility report generated - Overall score: ${overallScore.toFixed(1)}%`,
    );
    return report;
  }
  ...
  validateAccessibilityStandards(
    ...
    if (
      standards.minimumScore &&
      report.overallScore < standards.minimumScore
    ) {
      failures.push(
        `Overall score ${report.overallScore.toFixed(1)}% below minimum ${standards.minimumScore}%`,
      );
    }

    if (
      standards.maxViolations &&
      report.axeResults.violations.length > standards.maxViolations
    ) {
      failures.push(
        `${report.axeResults.violations.length} violations exceed maximum ${standards.maxViolations}`,
      );
    }
