...
export class TestDataManager {
  ...
  async initialize(): Promise<void> {
    this.logger.info(
      `Initializing test data for environment: ${this.environment}`,
    );
  ...
  private async loadTestData(): Promise<void> {
    try {
      ...
      if (fs.existsSync(dataFile)) {
        ...
        this.logger.warn(
          `Test data file not found: ${dataFile}. Using default data.`,
        );
   ...
   */
  getSearchQueriesByCategory(
    category: "valid" | "invalid" | "edge-case" | "performance",
  ) {
    return (
      this.testData?.searchQueries.filter(
  ...
  getSearchQueriesByBehavior(behavior: "pass" | "fail") {
    return (
      this.testData?.searchQueries.filter(
        (query) => query.expectedBehavior === behavior,
      ) || []
    );
  }
  ...
  getRandomSearchQuery(
    ...
    const queries = category
      ? this.getSearchQueriesByCategory(category)
