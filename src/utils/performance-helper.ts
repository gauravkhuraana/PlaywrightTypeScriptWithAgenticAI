...
export class PerformanceHelper {
  ...
  > {
    ...
    this.logger.info(
      `Resource timing collected: ${resources.length} resources`,
    );
    return resources;
  }
  ...
  async measureJavaScriptExecution(): Promise<{
    ...
    const jsMetrics = await this.page.evaluate(() => {
      ...
      const totalScriptTime = scripts.reduce(
        (sum, script) => sum + script.duration,
        0,
      );
      const scriptCount = scripts.length;
      const averageScriptTime =
        scriptCount > 0 ? totalScriptTime / scriptCount : 0;
  ...
  async measureCSSPerformance(): Promise<{
    ...
    const cssMetrics = await this.page.evaluate(() => {
      ...
      const totalCSSTime = stylesheets.reduce(
        (sum, css) => sum + css.duration,
        0,
      );
  ...
  validatePerformanceThresholds(
    ...
    if (thresholds.loadTime && metrics.loadTime > thresholds.loadTime) {
      failures.push(
        `Load time ${metrics.loadTime}ms exceeds threshold ${thresholds.loadTime}ms`,
      );
    }
    ...
    if (
      ...
      failures.push(
        `FCP ${metrics.firstContentfulPaint}ms exceeds threshold ${thresholds.firstContentfulPaint}ms`,
      );
    }
    ...
    if (
      ...
      failures.push(
        `LCP ${metrics.largestContentfulPaint}ms exceeds threshold ${thresholds.largestContentfulPaint}ms`,
      );
    }
    ...
    if (
      ...
      failures.push(
        `CLS ${metrics.cumulativeLayoutShift} exceeds threshold ${thresholds.cumulativeLayoutShift}`,
      );
    }
    ...
    if (
      ...
      failures.push(
        `FID ${metrics.firstInputDelay}ms exceeds threshold ${thresholds.firstInputDelay}ms`,
      );
    }
  ...
  async generatePerformanceReport(): Promise<{
    ...
    const [coreWebVitals, pageLoad, network, javascript, css, memory] =
      await Promise.all([
  ...
  async monitorPerformanceDuringAction<T>(
    ...
    const performanceData = {
      ...
      memoryDelta: {
        ...
        totalJSHeapSize:
          afterMemory.totalJSHeapSize - beforeMemory.totalJSHeapSize,
      },
    };

    this.logger.info(
      `Performance monitoring completed for ${actionName}:`,
      performanceData,
    );
