...
test.describe("API Integration Tests", () => {
  ...
  test("should test GET request with httpbin @api @smoke", async ({
    ...
    allure.description(
      "This test validates GET requests using httpbin.org testing service",
    );
    ...
    allure.attachment(
      "Query Parameters",
      JSON.stringify(response.data.args, null, 2),
      "application/json",
    );
    allure.attachment(
      "Response Headers",
  ...
  test("should handle API error responses @api @error-handling", async ({
    ...
    allure.description(
      "This test validates proper handling of API error responses",
    );
    allure.severity("normal");
    ...
    try {
      ...
      logger.info(
        `Error handling test caught expected error: ${String(error)}`,
      );
  ...
  test("should validate API response performance @api @performance", async ({
    ...
    allure.description(
      "This test validates API response performance meets requirements",
    );
    allure.severity("normal");
  ...
  test("should validate API response schema @api @schema", async ({
    ...
    allure.description(
      "This test validates API response adheres to expected schema",
    );
    allure.severity("normal");
    ...
    postsToValidate.forEach((post: any, index: number) => {
      ...
      Object.entries(postSchema).forEach(([field, expectedType]) => {
        if (!(field in post)) {
          ...
          issues.push(
            `Field ${field} has type ${typeof post[field]}, expected ${expectedType}`,
          );
          isValid = false;
        }
      });
      ...
      if (isValid) {
        ...
        schemaValidationResults.push(
          `Post ${postNumber}: Invalid schema - ${issues.join(", ")}`,
        );
        logger.error(
          `✗ Post ${postNumber} schema issues: ${issues.join(", ")}`,
        );
      }
    });

    // Count valid vs invalid
    const validCount = schemaValidationResults.filter((result) =>
    ...
    allure.attachment(
      "Posts Validated",
      postsToValidate.length.toString(),
      "text/plain",
    );
